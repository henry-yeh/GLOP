...
def solve_and_decompose(instance, opts):
    ...
    instance = instance.repeat(opts.width, 1, 1)
...
def generate_tsp_data(dataset_size, opts):
    batch_size = opts.width * (opts.graph_size//opts.revision_len) * opts.revision_len
    dataset = None
    ...
    return dataset.numpy().tolist()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", default='data/FI_train_tsp', help="Create datasets in data_dir")
    parser.add_argument("--name", default='FI20', type=str, help="Name to identify dataset")
    ...
    for idx in range(5):
        print('--------',idx,'---------')
        filename = os.path.join(datadir, "{}_{}_seed{}_{}.pkl".format(
        graph_size, opts.name, opts.seed, idx))
