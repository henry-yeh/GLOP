...
def generate_tsp_data_scale(dataset_size, tsp_size):
    if dataset_size != 10000:
        warning('Please note the distribution of tsp dataset (scale version) !')
    scale_dataset = []
    for y_max in np.linspace(0.1, 1, 10):
        scale_x = np.random.uniform(low=0, high=1, size=(dataset_size//10, tsp_size, 1))
        scale_y = np.random.uniform(low=0, high=y_max, size=(dataset_size//10, tsp_size, 1))
        scale_data = np.concatenate((scale_x, scale_y), axis=2)
        assert scale_data.shape == (dataset_size//10, tsp_size, 2)
        scale_data = scale_data.tolist()
        scale_dataset.extend(scale_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default='val', help="Name to identify dataset")
        ...
        for distribution in distributions or [None]:
            for graph_size in opts.graph_sizes:
                ...
                if problem == 'tsp':
                    ...
                    elif distribution == 'scale':
                        dataset = generate_tsp_data_scale(opts.dataset_size, graph_size)
                    else:
