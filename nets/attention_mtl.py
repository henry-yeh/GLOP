...
################### CONTEXT #################################
        self.init_embed_node = nn.Linear(node_dim, 128)
        self.init_embed_context = nn.Linear(10, embedding_dim - 128) # TODO !!!!!!
################### CONTEXT #################################
...
#################################### PREPROCESSING ######################        
        ...
        if return_pi and not self.training:
...
#################################### CONTEXT EMBEDDING ######################
        ...
        context_label, _ = torch.div(input[:,:,1], 0.1, rounding_mode='floor').max(dim=1)
        context_label = context_label.long()
        onehot_context = F.one_hot(context_label, num_classes=10).float()
        onehot_context = onehot_context.unsqueeze(1).repeat((1, input.shape[1], 1))
        # input = torch.concat((input, onehot_context), dim=2)
        init_embedding = self._init_embed(input, onehot_context)
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            embeddings, _ = checkpoint(self.embedder, init_embedding)
        else:
            embeddings, _ = self.embedder(init_embedding)
...
################################################################################
    ...
    def _init_embed(self, ini_node, ini_context):
...
################################# MTL #########################################
        return torch.cat(
            (self.init_embed_node(ini_node), self.init_embed_context(ini_context)),
            dim=-1
        )
################################# MTL #########################################
